...
def identificar_caras(embeddings: np.ndarray,
    ...
    for i in range(embeddings.shape[0]):
        ...
        for key, value in dic_referencia.items():
            similitudes[key] = 1 - cosine(np.ravel(embeddings[i]), np.ravel(value))
...
# Funciones para la lógica de los botones
def reconocimiento_imagenes():
    # Abrir el cuadro de diálogo para seleccionar una imagen
    fig, ax = plt.subplots(figsize=(12, 7))
    path_imagen = filedialog.askopenfilename(filetypes=[("Image files", "*.jpg;*.jpeg;*.png")])

    # Verificar si se seleccionó una imagen
    if path_imagen:
        imagen = Image.open(path_imagen)

        # Llamar a la función pipeline_deteccion_imagen con la imagen seleccionada
...
def reconocimiento_videos():
    # Abrir el cuadro de diálogo para seleccionar el video de entrada
    path_input_video = filedialog.askopenfilename(filetypes=[("Video files", "*.mp4")])

    # Verificar si se seleccionó un video de entrada
    if path_input_video:
        # Abrir el cuadro de diálogo para seleccionar el video de salida
        path_output_video = filedialog.asksaveasfilename(defaultextension=".mp4",
                                                         filetypes=[("Video files", "*.mp4")])

        # Verificar si se seleccionó un video de salida
        if path_output_video:
            # Llamar a la función pipeline_deteccion_video con los parámetros correspondientes
...
def reconocimiento_tiempo_real():
    # Llamar a la función pipeline_deteccion_webcam con los parámetros correspondientes
...
window.title("Trabajo Integrador Nº2 - Inteligencia Artificial")
window.configure(bg="#171375")  # Azul claro
...
# Establecer la posición inicial y dimensiones de la ventana
window.geometry(f"{window_width}x{window_height}+{x}+{y}")

# Crear un título grande alineado a la izquierda
titulo_label = tk.Label(window, text="Trabajo Integrador Nº2 - Inteligencia Artificial", font=("Segoe UI", 24, "bold"), fg="white", bg="#171375", anchor="w")
titulo_label.pack(padx=10, pady=10)

# Crear un texto de "Sistema de reconocimiento facial" alineado a la izquierda
subtitle_label = tk.Label(window, text="Sistema de reconocimiento facial", font=("Segoe UI", 20, "bold"), fg="white", bg="#171375", anchor="w")
subtitle_label.pack(padx=10, pady=10)

# Crear un texto de "Integrantes - Grupo 5" alineado a la izquierda
members_label = tk.Label(window, text="Integrantes - Grupo 5:", font=("Segoe UI", 14, "bold"), fg="white", bg="#171375", anchor="w")
members_label.pack(padx=10, pady=10)

members1_label = tk.Label(window, text="Lozano, Iñaki Fernando", font=("Segoe UI", 12), fg="white", bg="#171375", anchor="w")
members1_label.pack(padx=10, pady=0)

members2_label = tk.Label(window, text="Medina Raed, Luis Eugenio", font=("Segoe UI", 12), fg="white", bg="#171375", anchor="w")
members2_label.pack(padx=10, pady=0)

members3_label = tk.Label(window, text="Padros, Marcos Isaias", font=("Segoe UI", 12), fg="white", bg="#171375", anchor="w")
members3_label.pack(padx=10, pady=0)

# Crear los botones
button_frame = tk.Frame(window, bg="#171375")
button_frame.pack(pady=15)

button1 = tk.Button(button_frame, text="Reconocimiento en Imágenes", font=("Segoe UI", 14), fg="white", bg="#292755", relief="flat", borderwidth=2, padx=5, pady=5, command=reconocimiento_imagenes)
button1.pack(side="left", padx=8)

button2 = tk.Button(button_frame, text="Reconocimiento en Videos", font=("Segoe UI", 14), fg="white", bg="#292755", relief="flat", borderwidth=2, padx=5, pady=5,  command=reconocimiento_videos)
button2.pack(side="left", padx=8)

button3 = tk.Button(button_frame, text="Reconocimiento en Tiempo Real", font=("Segoe UI", 14), fg="white", bg="#292755", relief="flat", borderwidth=2, padx=5, pady=5, command=reconocimiento_tiempo_real)
button3.pack(side="left", padx=8)

# Agregar los logos centrados debajo
facet_logo = tk.PhotoImage(file="./images/2.png")
facet_label = tk.Label(window, image=facet_logo, bg="#171375")
facet_label.pack(pady=10)
